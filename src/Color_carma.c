...
/*****************************************************************************************************************************/
	...
	if ( argc == 1 || (HAVE_PDB == 0 && HAVE_DCD == 0 && HAVE_PSF == 0 && FILTER==NO))
		...
		printf("\n\033[37m\033[1mcarma v.2.4 \033[4;37m___________________________________________________________________\033[0m\n");
...
*/
	...
	else if ( strncasecmp( argv[i], "-TCR", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("\033[31m\033[1mTCROSS keyword only meaningful for a DCD-PSF run. Abort.\033[0m\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-MM", 3) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("\033[31m\033[1mMaxMin keyword only meaningful for a DCD-PSF run. Abort.\033[0m\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-NAT", 4) == 0 )
		{
                        if ( sscanf( argv[i+1], "%s", QFRACT_PDB ) != 1 )
                                {
                                        printf("\033[31m\033[1mProblem with the argument to \"-native\".\033[0m\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-PAC", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("\033[31m\033[1mPACK keyword only meaningful for a DCD-PSF run. Abort.\033[0m\n");
				myexit(1);
				}
	...
	if ( TCROSS == YES )
		{
			if (  HAVE_DCD == NO || HAVE_PSF == NO )
			{
			printf("\033[31m\033[1mThe \"-tCROSS\" flag only relevant for PSF-DCD.\033[0m\n");
			myexit(1);	
			}

			if ( HAVE_SEGID == NO )
			{
			printf("\033[31m\033[1mThe \"-tCROSS\" flag requires at least one segid selection.\033[0m\n");
			myexit(1);	
			}
									/* Force atom selection needed for dihedrals */
			if ( CHI1 == NO )
...
/*****************************************************************************************************************************/
	...
	i = 0;
        while ( fgets( string, 299, indeces ) != NULL )
		{
			if ( sscanf( string, "%d", &reposindex) != 1 )
				{	
					printf("\033[31m\033[1m\n\nThe 'reposition.index' file is not valid. Abort.\033[0m\n");
					myexit(1);
							}
			reposindex--;
			...
			if ( reposindex < 0 )
				{
					printf("\033[31m\033[1m\n\nNOTE : carma expects 1-based indeces in the reposition.index file.\033[0m\n");
					myexit(1);
							}

			if ( reposindex >= nofCAs )
				{	
					printf("\033[31m\033[1m\n\nToo large index in the 'reposition.index' file. Abort.\033[0m\n");
					myexit(1);
							}
	...
	if ( VERBOSE && CHI1 == YES && TCROSS == YES )
		printf("\033[33mIncluding chi1 angles in calculation of torsion RMSD matrix.\033[0m\n");
	...
	if ( headerframes == 0 )
		...
		{
			if ( VERBOSE )
			printf("\033[32m\033[1mThis looks like a new format NAMD-produced DCD file.\033[0m\n");
					}
		else
		{
			printf("\033[31m\033[1mZero number of frames and not the new DCD format. Giving up.\033[0m\n");
			myexit( 1 );
					}		
		}
	...
	if ( nofatoms != nofPSFatoms )
		{
			printf("\033[31m\033[1mNumber of atoms in PSF and DCD do not match (%d vs %d). Abort.\n", nofPSFatoms, nofatoms);
			myexit( 1 );
					}
...
#endif
				{
				printf("\033[33mLast frame set to %d\n\033[0m", headerframes );
				LAST = headerframes;
				}				
			if ( LAST < 0 )
				LAST = headerframes;
		...
		if ( QFRACTION == YES )
			{
				...
				if ( rms_file == NULL )
					{
						printf("\033[31m\033[1mCan not open carma.rms_from_reference.dat for writing. Abort.\033[0m\n");
						myexit(1);
					}
		...
		if ( QFRACTION == YES && HAVE_REF == NO )
			{
				if ( HAVE_PDB_QFRACTION == YES && REFERENCE > 1 )
					{
						printf("\033[31m\033[1mBoth -ref and -nat defined for Qfraction ? Abort.\033[0m\n");
						myexit(1);
					}
				...
				if ( atoms_from_pdb != nofCAs )
					{
					printf("\033[31m\033[1mNumber of atoms from PDB (%d) does not match the one from DCD (%d). Abort.\033[0m\n", atoms_from_pdb, nofCAs );
					myexit(1);
					}
				...
				if ( (unsigned int)(read( dcd, (void *)(&dcd_frame[1]), (size_t)(bytes_per_set) )) != bytes_per_set )
					{
						printf("\033[31m\033[1mCan't seek back to beginning of data ? Abort.\033[0m\n");
						myexit(1);
					}
				build_CAs( (int)(nofatoms) + 2);
				max = Dist();
	...
	i = 0;
        ...
        while ( fgets( string, 299, fmap_index ) != NULL )
		{
			if ( sscanf( string, "%d", &fmapindeces[i]) != 1 )
				{	
					printf("\033[31m\033[1m\n\nThe 'fmap.index' file is not valid. Abort.\033[0m\n");
					myexit(1);
							}
			fmapindeces[i] = fmapindeces[i] - 1;
			if ( fmapindeces[i] < 0 )
				{
					printf("\033[31m\033[1m\n\nNOTE : carma expects 1-based indeces in the fmap.index file.\033[0m\n");
					myexit(1);
							}

			if ( fmapindeces[i] < 0 )
				{	
					printf("\033[31m\033[1m\n\nNegative index in the 'fmap.index' file ? Abort.\033[0m\n");
					myexit(1);
							}
...
/*****************************************************************************************************************************/
	...
	fclose(rms_evolution);
	printf("\033[33mMax of mins of RMSD matrix is %f.\033[0m\n", max_of_mins );
	}
...
char	name[];
{
	...
	if ( in == NULL )
		...
		if ( strncasecmp( &string[13], &str_atname[nof][0], 1 ) != 0 )
			{
			printf("\n\033[31m\033[1mMismatch between PSF and native structure's PDB ? Abort.\033[0m\n\n");
			myexit(1);
			}
		}
...
{
	printf("\n\n%s\n\n", &error_text[0] );
	printf("\033[31m\033[1mAllocation errors mean that carma requested additional\033[0m\n");
	printf("\033[31m\033[1mmemory and the operating system refused to offer it.\033[0m\n");
	printf("\033[31m\033[1mIf your computing machine does have enough memory for\033[0m\n");
	printf("\033[31m\033[1mthe envisioned calculation, getting the 64bit GNU/Linux\033[0m\n");
	printf("\033[31m\033[1mcarma executable may be the solution to this problem.\033[0m\n\n");
...
float	**data;
{
	...
	printf("\033[31m\033[1m\n\nEntropy calculation will ignore negative eigenvalues !\033[0m\n");
...
float bestfit(rotate_t u, xlate_t t,
	...
	i = 0;
        while ( fgets( string, 299, indeces ) != NULL )
		{
			...
			if ( sscanf( string, "%d", &index[i]) != 1 )
				{	
					printf("\033[31m\033[1m\n\nThe 'fit.index' file is not valid. Abort.\033[0m\n");
					myexit(1);
							}
			index[i] = index[i] - 1;
			if ( index[i] < 0 )
				{
					printf("\033[31m\033[1m\n\nNOTE : carma expects 1-based indeces in the fit.index file.\033[0m\n");
					myexit(1);
							}

			if ( index[i] >= npts )
				{	
					printf("\033[31m\033[1m\n\nToo large index in the 'fit.index' file. Abort.\033[0m\n");
					myexit(1);
							}
			i++;
			}
  ...
  sw = solve(e, e0, det, spur, cof);
	...
	if ( sw != 1 )
		{
                printf("\033[31m\033[1m Internal problem with bestfit(). Continuing (but beware) ...\033[0m\n");
                }
	...
	for ( i=0 ; i < nofCAs ; i++ )
		{
			if ( 	strncmp( &backbone  [i][5], " N   ", 5) != 0	&&
				...
				strncmp( &backbone  [i][5], " C   ", 5) != 0	)
					{
					printf("\033[31m\033[1mUnexpected atom for torsions : %s\033[0m\n" ,&backbone[i][5] );
					myexit( 1 );
					}
...
*/
	...
	if ( tot_torsions < 2 )
		{
			printf("\033[31m\033[1mLess than two torsions left after selection ? Abort.\033[0m\n");
			myexit( 1 );
					}
	...
	printf("\033[33mt-RMSD matrix calculation : %d torsion angles included\033[0m\n", tot_torsions);
...
int	build_torsions_with_chi()
{
	...
	if ( nofCAs < 6 )
		{
			printf("\033[31m\033[1mToo few atoms selected for torsions ? Abort.\033[0m\n");
			myexit( 1 );
					}
...
*/
	...
	if ( VERBOSE && TCROSS == YES )
		{
		printf("\033[33mt-RMSD matrix calculation : %d torsion angles\033[0m\n", tot_torsions);
		printf("\033[33mof which %d are backbone and %d are chi1 angles.\033[0m\n", tot_back, tot_side );
		}
...
int	bins;
{
	...
	if ( x < 2 || x > bins-1 ||  y < 2 || y > bins-1 || z < 2 || z > bins-1 )
	{
		...
		printf("\033[33m[this probably means that the number of\nframes used for the analysis is too small].\033[0m\n");
		exit(1);
	}
