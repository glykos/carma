...
/**															    **/
/**	Version 1.3 :	- Fix carma.superposition.pdb so that it displays properly with rasmol  			    **/
/**			- Fix number of frames written in the header of output DCD files				    **/
/**			- '-Qfraction' now cooperates with '-ref'							    **/
/**			- '-Qfraction' can now use a pdb file as a source of a native structure (flag '-nat')		    **/
/**			- '-stride' works with macosx as well								    **/
/**			- carma.selected_atoms.psf now renumber the atoms written in the file				    **/
/**			- Tidy-up messages to play along with grcarma without problems					    **/
/**			- In graphics mode key 'L' displays labels							    **/
/**			- Try to fix torsion-angle enumeration for homo-multimeric proteins				    **/
/**															    **/
/**	Version 1.4:	- Fixed a windoze-specific bug in the handling of large files.					    **/
/**															    **/
/**	Version 1.5:	- Flag '-force' to complete the entropy calculation no matter what.				    **/
/**															    **/
/**	Version 1.6:    - Process correctly new DCD header format from NAMD						    **/
/**			- Use tanh() to reduce the height of peaks in 3D clustering.					    **/
/**															    **/
/**	Version 1.7:	- Freedman-Diaconis rule for estimating number of bins for clustering				    **/
/**			- Improved (?) heurestics for clustering cutoff							    **/
/**															    **/
/**	Version 1.8:	- Flag '-mm' modified : if '-first' is given, assume that the user wants to calculate cross	    **/
/**			  RMSD matrix minima (see https://norma.mbg.duth.gr/index.php?id=research:howto:                    **/
/**			  comparing_two_secondary_structure_distributions#via_cross_rmsd_matrices_cartesian_or_torsion )    **/
/**			- Flag '-tcross' added for calculating torsion-RMSD matrices 					    **/
/**			- Flag '-pack' to calculate when preparing PDB files minimum PBC-related distances between images   **/
/**															    **/
/**															    **/
/**															    **/
/**															    **/
...
/*****************************************************************************************************************************/
	...
	if ( k > 0 )
		{
			printf("_REDB_A lone postscript file as input ? Abort._DEF_\n");
			myexit( 1 );
					}
	...
	if ( strncasecmp( &argv[i][ strlen( argv[i]) - 4], ".PDB", 4) == 0 )
		{
			...
			if ( sscanf( argv[i], "%f", &junkf ) == 1 )
				printf("_YELLOW_Warning: PDB filename looks like a number._DEF_\n");
		}
	if ( strncasecmp( &argv[i][ strlen( argv[i]) - 4], ".DCD", 4) == 0 )
		{
			HAVE_DCD = i;
			if ( sscanf( argv[i], "%f", &junkf ) == 1 )
				printf("_YELLOW_Warning: DCD filename looks like a number._DEF_\n");   
		}                          
	if ( strncasecmp( &argv[i][ strlen( argv[i]) - 4], ".PSF", 4) == 0 )
		{
			HAVE_PSF = i;
			if ( sscanf( argv[i], "%f", &junkf ) == 1 )
				printf("_YELLOW_Warning: PSF filename looks like a number._DEF_\n");   
			...
			for ( k=1 ; k < argc ; k++)
				if ( strncasecmp( &argv[k][0], "-W", 2) == 0 )
					{
					printf("_REDB_\nRefusing to overwrite the input PSF file (carma.selected_atoms.psf). Please rename._DEF_\n\n");
					myexit(1);
					}
			}
		...
		if ( strncasecmp( argv[i], "-SO", 3) == 0 )
			{
				if ( HAVE_DCD > 0 )
					...
					{
					printf("_REDB_\"-sort\" without a DCD file specification ?_DEF_\n");
					myexit(1);
					}
			}
...
/*****************************************************************************************************************************/
	...
	if ( argc == 1 || (HAVE_PDB == 0 && HAVE_DCD == 0 && HAVE_PSF == 0 && FILTER==NO))
		...
		printf("\n_UWHITE______________________________DEF__WHITE_ Helper applications _DEF__UWHITE________________________________DEF_\n\n");

		printf("_GREEN_carma -col -_DEF_\n");
		printf("\tRead [n*n] values from stdin, and make a color postscript bitmap.\n");

		printf("_GREEN_carma <first.ps> <second.ps>_DEF_\n");
		printf("\tRead two carma-made postscript files and make a composite image.\n");

		printf("_GREEN_carma -sort <frame_order.list> <my.dcd>_DEF_\n");
		printf("\tReorder frames in DCD file to match the order in <frame_order.list>.\n");

		printf("_UWHITE_________________________________________________________________________________DEF_\n\n");
...
/*
			if ( HAVE_PDB != NO )
				printf("_REDB_Frame specification for a .pdb file ? Ignored._DEF_\n");
*/
				
                        if ( sscanf( argv[i+1], "%d", &FIRST ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-first\"._DEF_\n");
					myexit( 1 );
                                                        }
			i++;
                        if ( FIRST <= 0 )
                        	{
                        		printf("_REDB_Frame numbers must be greater than zero._DEF_\n");
                        		myexit(1);
                        				}
...
*/
                        ...
                        if ( sscanf( argv[i+1], "%d", &LAST ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-last\"._DEF_\n");
					myexit( 1 );
                                                        }
			i++;
                        if ( LAST <= 0 )
	...
	else if ( strncasecmp( argv[i], "-AT", 3) == 0 )
		{
			char	inter[6] = "     ";
                        ...
                        if ( sscanf( argv[i+1], "%s", &junk[0] ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-atomid\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
                        ...
                        if ( strlen( argv[i] ) > 5 )
                        	{
                        		printf("_REDB_Atom specification error : string (%s) too long._DEF_\n", argv[i]);
                        		myexit(1);
                        				}
	...
	else if ( strncasecmp( argv[i], "-SE", 3) == 0 )
		{
			...
			if ( HAVE_PDB == YES || HAVE_PSF == NO )
				{
				printf("_REDB_SegID specification only valid for PSF-DCD specification (not PDB). Abort._DEF_\n");
				myexit(1);
				}
				
                        if ( sscanf( argv[i+1], "%s", &junk[0] ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-segid\"._DEF_\n");
					myexit( 1 );
                                                        }
			...
			i++;
                        if ( strlen( argv[i] ) > 5 )
                        	{
                        		printf("_REDB_SegID specification error : string (%s) too long._DEF_\n", argv[i] );
                        		myexit(1);
                        				}
                        if ( strlen( argv[i] ) < 5 )
				strncpy( &inter[1], argv[i], strlen(argv[i]) );
			...
			if ( HAVE_SEGID == YES )
				{
					for ( m=0 ; m < strlen( SEGID ) ; m += 5 )
						if ( strncasecmp( &inter[0], &SEGID[m], 5) == 0 )
							{
							printf("_REDB_Same SEGID given twice ? (%s). Abort._DEF_\n", inter);
							myexit(1);
							}
				}
	...
	else if ( strncasecmp( argv[i], "-FIT", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_FIT keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-CRO", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_CROSS keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-TCR", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_TCROSS keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-MM", 3) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_MaxMin keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-QF", 3) == 0 )
		{
                        if ( sscanf( argv[i+1], "%f", &Q_CUTOFF ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-Qfraction\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-NAT", 4) == 0 )
		{
                        if ( sscanf( argv[i+1], "%s", QFRACT_PDB ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-native\"._DEF_\n");
					myexit( 1 );
                                                        }

			HAVE_PDB_QFRACTION = YES;
                        i++;
                                                        	}
	else if ( strncasecmp( argv[i], "-PDB", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_PDB keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-PAC", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_PACK keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-SUP", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_'-super' keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-STR", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_STRIDE keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-NOF", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_NOFIT keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-I", 2) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_INDEX keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-PDD", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_PDDF keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-3D", 3) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_3D keyword only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}
	...
	else if ( strncasecmp( argv[i], "-SI", 3) == 0 )
		{
                        ...
                        if ( sscanf( argv[i+1], "%f", &LAMBDA ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-sigmoidal\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
			...
			if ( sscanf( argv[i+1], "%d", &DISTFIRST ) != 1 )	/* Is it a file with pairs ? */
				{
					...
					if ( test == NULL )
		                                {
        		                                printf("_REDB_Problem with the argument to \"-distance\"._DEF_\n");
							myexit( 1 );
                                        	 	               }
                                        else
                                        {
			...
			if ( sscanf( argv[i+1], "%d", &TORSFIRST ) != 1 )	/* Is it a file with tetraplets ? */
				{
					...
					if ( test == NULL )
		                                {
        		                                printf("_REDB_Problem with the argument to \"-torsion\"._DEF_\n");
							myexit( 1 );
                                        	 	               }
                                        else
                                        {
			...
			if ( sscanf( argv[i+1], "%d", &BENDFIRST ) != 1 )	/* Is it a file with triplets ? */
				{
					...
					if ( test == NULL )
		                                {
        		                                printf("_REDB_Problem with the argument to \"-bend\"._DEF_\n");
							myexit( 1 );
                                        	 	               }
                                        else
                                        {
	...
	else if ( strncasecmp( argv[i], "-G", 2) == 0 )
		{
                        ...
                        if ( sscanf( argv[i+1], "%f", &GRABOX ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-graphics\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
			GRABOX = fabs(GRABOX);
	...
	else if ( strncasecmp( argv[i], "-REF", 4) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
				printf("_REDB_Reference frame specification only meaningful for a DCD-PSF run. Abort._DEF_\n");
				myexit(1);
				}

                        if ( sscanf( argv[i+1], "%d", &REFERENCE ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-reference\"._DEF_\n");
					myexit( 1 );
                                                        }
			if ( sscanf( argv[i+2], "%d", &REF_STEP ) == 1  )
				{
					if ( VERBOSE )
					printf("_GREEN_Incremental rmsds requested. Step is %d._DEF_\n", REF_STEP );
					i++;
				}
                        i++;
                        if ( REFERENCE <= 0 )
                        	{
                        		printf("_REDB_Reference frame number must be a positive integer._DEF_\n");
                        		myexit(1);
                        				}
...
*/
                        ...
                        if ( sscanf( argv[i+1], "%d", &STEP ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-step\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
                        if ( STEP <= 0 )
                        	{
                        		printf("_REDB_Frame step numbers must be positive integers._DEF_\n");
                        		myexit(1);
                        				}
	...
	else if ( strncasecmp( argv[i], "-MI", 3) == 0 )
		{
			HAVE_MIN = YES;
                        if ( sscanf( argv[i+1], "%f", &gmin ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-min\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
                                                        	}
	else if ( strncasecmp( argv[i], "-MAX", 4) == 0 )
		{
			HAVE_MAX = YES;
                        if ( sscanf( argv[i+1], "%f", &gmax ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-max\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
                                                        	}
	else if ( strncasecmp( argv[i], "-DG", 3) == 0 )
		{
                        if ( sscanf( argv[i+1], "%f", &DGLEN ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-DGwidth\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
                                                        	}
	else if ( strncasecmp( argv[i], "-COL", 4) == 0 )
		{
			...
			if ( argc < (i+16) )
                                {
                                        printf("_REDB_Flag \"-colour\" expects 15 floats._DEF_\n");
					myexit( 1 );
                                                        }

                        if ( sscanf( argv[i+1], "%f", &RED_1 ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-colour\"._DEF_\n");
					myexit( 1 );
                                                        }
			...
			if ( 	RED_1 < 0.0 || RED_1 > 1.0 || RED_2 < 0.0 || RED_2 > 1.0 ||
				...
				BLUE_1 < 0.0 || BLUE_1 > 1.0 || BLUE_2 < 0.0 || BLUE_2 > 1.0 ||
                                ...
                                {
                                        printf("_REDB_Arguments to \"-colour\" must be between 0.0 and 1.0._DEF_\n");
					myexit( 1 );
                                                        }
		...
		{
			if ( HAVE_PDB != NO )
				printf("_REDB_Frame rmsd specification for a .pdb file ? Ignored._DEF_\n");
                        if ( sscanf( argv[i+1], "%f", &MAX_RMSD ) != 1 || MAX_RMSD <= 0.0 )
                                {
                                        printf("_REDB_Problem with the argument to \"-mrms\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
                                                        	}
	else if ( strncasecmp( argv[i], "-MAP", 4) == 0 )
		{
                        if ( sscanf( argv[i+1], "%f", &Xmin ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-map\"._DEF_\n");
					myexit( 1 );
                                                        }
			...
			if ( Xmin == Xmax || Ymin == Ymax || Zmin == Zmax )
                                {
                                        printf("_REDB_Only 3D density maps are supported._DEF_\n");
					myexit( 1 );
                                                        }

			if ( Grid <= 0.0 )
                                {
                                        printf("_REDB_Problem with the grid argument to \"-map\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-CRY", 4) == 0 )
		{
                        if ( sscanf( argv[i+1], "%lf", &f2o_11 ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-crystal\"._DEF_\n");
					myexit( 1 );
                                                        }
...
*/			
	...
	else if ( strncasecmp( argv[i], "-FM", 3) == 0 )
		{
                        if ( sscanf( argv[i+1], "%f", &Xmin ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-fmap\"._DEF_\n");
					myexit( 1 );
                                                        }
                        if ( sscanf( argv[i+2], "%f", &Xmax ) != 1  )
                                {
			...
			if ( Grid <= 0.0 )
                                {
                                        printf("_REDB_Problem with the grid argument to \"-fmap\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-PR", 3) == 0 )
		{
                        ...
                        if ( sscanf( argv[i+1], "%d", &EIGENLAST ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-proj\"._DEF_\n");
					myexit( 1 );
                                                        }
			...
			if ( strncasecmp( argv[i+4], "-", 1) != 0 && sscanf( argv[i+4], "%f", &CLU_RMS ) == 1  )
				{
					if ( VERBOSE )
					printf("_GREEN_Cutoff for cluster selection set to mean%+f sigma._DEF_\n", CLU_RMS );
					i++;
				}
			...
			EIGENSTEP  = 1;
                        ...
                        if ( EIGENLAST < 2 )
                                {
                                        printf("_REDB_Less than two eigenvectors requested for \"-proj\" ? Abort._DEF_\n");
					myexit( 1 );
                                                        }
			...
			if ( DGTEMP < 10.0 )
                                {
                                        printf("_REDB_It is cold out there ... Check the man page for \"-proj\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-DP", 3) == 0 )
		{
                        ...
                        if ( sscanf( argv[i+1], "%d", &EIGENLAST ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-dPCA\"._DEF_\n");
					myexit( 1 );
                                                        }
                        if ( sscanf( argv[i+2], "%d", &DG_PLOTS ) != 1  )
                                {
			...
			EIGENSTEP  = 1;
                        ...
                        if ( EIGENLAST < 2 )
                                {
                                        printf("_REDB_Less than two eigenvectors requested for \"-dPCA\" ? Abort._DEF_\n");
					myexit( 1 );
                                                        }
			...
			if ( DGTEMP < 10.0 )
                                {
                                        printf("_REDB_It is cold out there ... Check the man page for \"-dPCA\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-PL", 3) == 0 )
		{
                        if ( sscanf( argv[i+1], "%d", &EIGENPLAY ) != 1 || EIGENPLAY < 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-play\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-O", 2) == 0 )
		{
                        if ( sscanf( argv[i+1], "%d", &EIGENFIRST_W ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-out\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-AR", 3) == 0 )
		{
                        if ( sscanf( argv[i+1], "%d", &ART_EIGEN ) != 1  )
                                {
                                        printf("_REDB_Problem with the argument to \"-artificial\"._DEF_\n");
					myexit( 1 );
                                                        }
	...
	else if ( strncasecmp( argv[i], "-TE", 3) == 0 )
		{
			ENTROPY = YES;
                        if ( sscanf( argv[i+1], "%f", &Temp ) != 1 )
                                {
                                        printf("_REDB_Problem with the argument to \"-temp\"._DEF_\n");
					myexit( 1 );
                                                        }
                        i++;
                                                        	}
	...
	else if ( strncasecmp( argv[i], "-FO", 3) == 0 )
		{
			FORCE_ENTROPY = YES;
		}
#endif
		...
		{
			if ( HAVE_PDB != NO )
				printf("_REDB_Covariance request for .pdb file ? Ignored._DEF_\n");
			FIND_COV = YES;
		}
		...
		{
			printf("_REDB_Unidentified keyword or argument : %s. Abort.\n", argv[i] );
			myexit( 1 );
					}
	}
...
/*****************************************************************************************************************************/
	...
	if ( HAVE_PDB > 0 && HAVE_PDB_QFRACTION == YES )	/* Ignore this PDB. We only need it for fraction of native contacts */
		{
			HAVE_PDB = 0;
		}
	...
	if ( FILTER==NO && HAVE_PDB != NO && ( HAVE_DCD != NO || HAVE_PSF != NO ) )
		{
			printf("_REDB_Too many input files. You should either define a PDB alone,\nor a DCD plus its PSF._DEF_\n");
			myexit(1);
		}
	...
	if ( STRIDE == YES )
		{
			printf("_REDB_Sorry. \"-stride\" is only functional on unix machines._DEF_\n");
			myexit(1);	
		}
#endif
	...
	if ( STRIDE == YES && WRITE_PDB != YES )
		{
			printf("_REDB_The \"-stride\" flag requires \"-pdb\" as well._DEF_\n");
			myexit(1);	
					}
	...
	if ( ( HAVE_DCD != NO && HAVE_PSF == NO ) || ( HAVE_DCD == NO && HAVE_PSF != NO ))
		{
			printf("_REDB_For a PSF-DCD run both a PSF and a DCD file are needed._DEF_\n");
			myexit(1);	
					}
	...
	if ( DPCA == YES )
		{
			...
			{
			printf("_REDB_The \"-dPCA\" flag only relevant for PSF-DCD._DEF_\n");
			myexit(1);	
			}

			if ( MASSES == YES )
			{
			printf("_YELLOW_Does the \"-masses\" + \"-dPCA\" flags make sense ? Continue._DEF_\n");
			}

			if ( HAVE_SEGID == NO )
			{
			printf("_REDB_The \"-dPCA\" flag requires at least one segid selection._DEF_\n");
			myexit(1);	
			}
	...
	if ( TCROSS == YES )
		{
			if (  HAVE_DCD == NO || HAVE_PSF == NO )
			{
			printf("_REDB_The \"-tCROSS\" flag only relevant for PSF-DCD._DEF_\n");
			myexit(1);	
			}

			if ( HAVE_SEGID == NO )
			{
			printf("_REDB_The \"-tCROSS\" flag requires at least one segid selection._DEF_\n");
			myexit(1);	
			}
									/* Force atom selection needed for dihedrals */
			if ( CHI1 == NO )
	...
	if ( CALC_MAP == YES && ( HAVE_DCD == NO || HAVE_PSF == NO ))
		{
			printf("_REDB_The \"-map\" flag only relevant for PSF-DCD._DEF_\n");
			myexit(1);	
					}

	if ( GYRATION == YES && ( HAVE_DCD == NO || HAVE_PSF == NO ))
		{
			printf("_REDB_The \"-rg\" flag only relevant for PSF-DCD._DEF_\n");
			myexit(1);	
					}
	if ( ASA == YES )
		{
			if ( HAVE_DCD == NO || HAVE_PSF == NO )
			{
			printf("_REDB_The \"-surf\" flag only relevant for PSF-DCD._DEF_\n");
			myexit(1);	
			}
			if ( HAVE_SEGID == NO )
			{
			printf("_REDB_The \"-surf\" flag requires at least one segid selection._DEF_\n");
			myexit(1);	
			}
	...
	if ( MAX_RMSD >= 0.0 && FIND_RMS == NO )
		printf("_REDB_\"-mrms\" specified but not \"-rms\" ? Ignored._DEF_\n");


	if ( FIND_RMS == YES && FIND_COV == YES )
		{
			printf("_REDB_Can not calculate rms and covariance matrices in one pass._DEF_\n");
			myexit(1);	
					}

	if ( (USE_DOT == YES || NORMALISE == YES) && FIND_COV == NO )
		printf("_REDB_The \"-norm\" and \"-dot\" flags are not relevant. Ignored._DEF_\n");

	if ( DO_FIT != YES && HAVE_INDEX == YES )
		{
		printf("_REDB_Have \"-index\" flag in the absence of \"-fit\" ? Abort._DEF_\n");
		myexit(1);
		}
	...
	if ( MASSES == YES && FIND_COV == NO && CALC_MAP == NO )
		{
		printf("_REDB_The \"-mass\" flag without \"-cov\" option ? Abort._DEF_\n");
		myexit(1);
		}

	if ( FAST_MAP && CALC_MAP )
		{
		printf("_REDB_Use either \"-map\" or \"-fmap\". Abort._DEF_\n");
		myexit(1);
		}
	if ( FAST_MAP && !DO_FIT )
		{
		printf("_REDB_\"-fmap\" requires \"-fit\". Abort._DEF_\n");
		myexit(1);
		}
	if ( CALC_MAP && DO_FIT )
		{
		printf("_REDB_Use \"-fmap\" instead of \"-map\" if you want to \"-fit\". Abort._DEF_\n");
		myexit(1);
		}
	...
	if ( (EIGEN == YES || EIGENFIRST > 0 || EIGENFIRST_W > 0 || EIGENPLAY > 0 ) && FIND_COV == NO && DPCA == NO)
		{
		printf("_REDB_The \"-cov\" flag is needed for principal component analysis._DEF_\n");
		myexit(1);
		}

	if ( EIGEN == NO && ( EIGENFIRST > 0 || EIGENFIRST_W > 0 || EIGENPLAY > 0 ) && DPCA == NO )
		{
		printf("_REDB_The \"-eigen\" flag is also needed for the intended calculation._DEF_\n");
		myexit(1);
		}

	if ( EIGEN == YES && (USE_DOT == YES || NORMALISE == YES) )
		{
		printf("_REDB_The \"-dot\" and/or \"-norm\" flags are mutually exclusive with the \"-eigen\" flag._DEF_\n");
		myexit(1);
		}
	if ( EIGEN == NO && USE_LAST == YES )
		{
		printf("_REDB_The \"-use\" flag is meaningless in the absence of the \"-eigen\" option._DEF_\n");
		myexit(1);
		}
	if ( ENTROPY == YES && ( MASSES == NO || FIND_COV == NO || EIGEN == NO ))
		{
		printf("_REDB_Entropy calculation requires \"-cov\", \"-eigen\" and \"-masses\"._DEF_\n");
		myexit(1);
		}
	if ( EIGENPLAY > 0 && USE_DOT == YES )
		{
			printf("_REDB_Can not do eigenvector projection with \"-dot\" products._DEF_\n");
			exit ( 1 );
					}
#endif				
...
/*****************************************************************************************************************************/
	...
	if ( nofCAs < 2 )
		{
			if ( HAVE_SEGID == NO )
				printf("_REDB_Less than two \"%s\" atoms found ? Abort._DEF_\n", ATOMID);
			else
				printf("_REDB_Less than two \"%s\" atoms found for segid \"%s\". Abort._DEF_\n", ATOMID, SEGID);
	...
	if ( VERBOSE )
		{
		end_timer = time(NULL);
		printf("_GREENB_All done in %1.1f minutes._DEF_\n", (float)(end_timer-start_timer)/60.0 );
		}
#endif
#ifdef WINDOWS
	if ( VERBOSE )
		{
		printf("_GREENB_All done._DEF_\n" );
		}
#endif
...
/*****************************************************************************************************************************/
	...
	{
		printf("_REDB_You need to define both a .dcd and a .psf file._DEF_\n");
		myexit( 1 );
				}
	...
	if ( VERBOSE )
		printf("_CYANB_Will change the order of atoms._DEF_\n");


	indeces = fopen("reposition.index", "r");
	if ( indeces == NULL )
		{	
			printf("_REDB_Can not open the 'reposition.index' file. Abort._DEF_\n");
			myexit(1);
					}
	...
	i = 0;
        while ( fgets( string, 299, indeces ) != NULL )
		{
			if ( sscanf( string, "%d", &reposindex) != 1 )
				{	
					printf("_REDB_\n\nThe 'reposition.index' file is not valid. Abort._DEF_\n");
					myexit(1);
							}
			reposindex--;
			...
			if ( reposindex < 0 )
				{
					printf("_REDB_\n\nNOTE : carma expects 1-based indeces in the reposition.index file._DEF_\n");
					myexit(1);
							}

			if ( reposindex >= nofCAs )
				{	
					printf("_REDB_\n\nToo large index in the 'reposition.index' file. Abort._DEF_\n");
					myexit(1);
							}
	...
	if ( VERBOSE && CHI1 == YES && DPCA == YES )
		printf("_YELLOW_Including chi1 angles in dihedral PCA._DEF_\n");

	if ( VERBOSE && CHI1 == YES && TCROSS == YES )
		printf("_YELLOW_Including chi1 angles in calculation of torsion RMSD matrix._DEF_\n");


	if ( VERBOSE )
		{ 
		if ( !HEAVY && !ALL_ATID )
			printf("_YELLOW_%d %s atoms are declared in the PSF file._DEF_\n", nofCAs, ATOMID);
		else if ( HEAVY )
			printf("_YELLOW_%d non-hydrogen atoms are declared in the PSF file._DEF_\n", nofCAs);
		else if ( ALL_ATID )
			printf("_YELLOW_%d atoms are declared in the PSF file._DEF_\n", nofCAs);
		}
	...
	if ( dcd == -1 )
		{
			printf("_REDB_Can not open DCD file for read. Abort._DEF_\n");
			myexit( 1 );
					}

	if ( read( dcd, dcd_head, (size_t)(92) ) != 92 )
		{
			printf("_REDB_Premature EOF for DCD file ? Abort._DEF_\n");
			myexit(1);
					}

	if ( strncasecmp( (char *)(&dcd_head[4]), "CORD", 4) != 0 )
		{
			printf("_REDB_Not a coordinate DCD file ? Abort._DEF_\n");
			myexit(1);
					}
	...
	if ( 	(unsigned int)(*((unsigned int *)(&dcd_head[48]))) == 1 ||
		(unsigned int)(*((unsigned int *)(&dcd_head[48]))) == 2  )
		{
			...
			CELL_OFFSET = 14;
			if ( VERBOSE )
				printf("_GREENB_It appears that this DCD file contains unit cell information._DEF_\n");
					}

 
	if (    (unsigned int)(*((unsigned int *)(&dcd_head[40]))) != 0 )
               {
			printf("_REDB_DCD files with fixed atoms are not supported. Abort._DEF_\n");
			myexit(1);
                                       }
	...
	if ( (unsigned int)(*((unsigned int *)(&dcd_head[52]))) == 1 )
		{
			printf("_REDB_It appears that this is a CHARMM-specific DCD file. Abort._DEF_\n");
			myexit(1);
					}
	...
	if ( 	(int)(*((int *)(&dcd_head[8]))) < 0 || 
		...
		{
			printf("_REDB_Negative number of frames ? Wrong endian for DCD file ?_DEF_\n");
			myexit( 1 );
					}

	if ( headerframes == 0 )
		{
		headerframes =    (unsigned int)(*((unsigned int *)(&dcd_head[20]))) 
				- (unsigned int)(*((unsigned int *)(&dcd_head[12])))
				+ (unsigned int)(*((unsigned int *)(&dcd_head[16])));

		if ( headerframes > 0 )
		{
			if ( VERBOSE )
			printf("_GREENB_This looks like a new format NAMD-produced DCD file._DEF_\n");
					}
		else
		{
			printf("_REDB_Zero number of frames and not the new DCD format. Giving up._DEF_\n");
			myexit( 1 );
					}		
		}
	...
	if ( FIRST > 0 && FIRST > headerframes )
		{
			printf("_REDB_Requested first frame does not exist. Abort._DEF_\n");
			myexit(1);
					}
#ifdef GRA
	if ( AUTOGRA == YES )
		...
		printf("_GREENB_Total number of frames (from header) is %d_DEF_\n", headerframes );
		printf("First frame to display [1] : ");
		fgets(s, 99, stdin);
		FIRST = atoi( s );
		if ( FIRST > headerframes || FIRST < 1 )
			FIRST = 1;
		}
	...
	if ( nofatoms != nofPSFatoms )
		{
			printf("_REDB_Number of atoms in PSF and DCD do not match (%d vs %d). Abort.\n", nofPSFatoms, nofatoms);
			myexit( 1 );
					}
	...
	*((unsigned int *)(&wdcd_frame[6+3*nofCAs])) = (unsigned int)( nofCAs*4);
...
#ifdef GRA
			if ( LAST > headerframes && AUTOGRA == NO && VERBOSE )
#endif
#ifndef GRA
			if ( LAST > headerframes && VERBOSE )
#endif
				{
				printf("_YELLOW_Last frame set to %d\n_DEF_", headerframes );
				LAST = headerframes;
				}				
			if ( LAST < 0 )
				LAST = headerframes;
	...
	if ( GRAPHICS)
	{
        ...
        if ( winid == -1 )
                {
                        printf("_REDB_No more windows available ? Aborting._DEF_\n");
                        myexit( 1 );
                                                }
        winset ( winid );
...
/*****************************************************************************************************************************/
	...
	if ( 	FIND_RMS == NO && FIND_COV == NO && DO_FIT == NO && CROSSDCD == NO && TCROSS == NO &&
	...
	{	
		...
		if ( FIRST > 1 )
			if ( lseek( dcd, (off_t)(bytes_per_set * (off_t)(FIRST-1)), SEEK_CUR ) == (off_t)(-1) )
				{
					printf("_REDB_Requested frame not present ? Abort._DEF_\n");
					myexit(1);
							}
		...
		if ( (unsigned int)(read( dcd, (void *)(&dcd_frame[1]), (size_t)(bytes_per_set) )) != bytes_per_set )
			{
				...
				if ( lseek( dcd, (off_t)(start), SEEK_SET ) == (off_t)(-1) )
					{
					printf("_REDB_Can not seek back to the beginning of data ? Abort._DEF_\n");
					myexit(1);
						}
				...
				if ( frame == FIRST )
					{
						printf("_REDB_\nRequested frame not present ? Abort._DEF_\n");
						myexit(1);
								}
				...
				if ( VERBOSE )
					{
					end_timer = time(NULL);
					printf("_GREENB_\nEnd of DCD reached. All done in %1.1f minutes._DEF_\n", (float)(end_timer-start_timer)/60.0 );
					}
#endif
#ifdef WINDOWS
	if ( VERBOSE )
...
#ifndef WINDOWS				
				end_timer = time(NULL);
				printf("_GREENB_\nAll done in %1.1f minutes._DEF_\n", (float)(end_timer-start_timer)/60.0);
#endif
#ifdef WINDOWS
	if ( VERBOSE )
		{
		...
		if ( HAVE_DISTANCE_FILE == YES )
		{
			...
			if ( in == NULL )
				{
        	                        printf("_REDB_Can not open file to read distance pairs._DEF_\n");
					myexit( 1 );
				}
			while ( fgets( string, 299, in ) != NULL )
			{
				...
				if ( at1 < 0 || at2 < 0 || at1 > nofCAs || at2 > nofCAs )
					{
						printf("_REDB_Requested atom index out of range. Abort._DEF_\n");
						myexit(1);
								}
				...
				if ( tot_dist > MAX_NOF_BACK-2 )
					{
						printf("_REDB_Too many atom pairs for distances. Increase MAX_NOF_BACK._DEF_\n");
						myexit(1);
								}
				}
			...
			if ( VERBOSE )
			{
				printf("_GREENB_Read %d pairs for distance calculation._DEF_\n", tot_dist );
			}
		}
		else
		{
		...
		dist = fopen("carma.distances", "w");
	        if ( dist == NULL )
        	        {
                		printf("_REDB_Can not open file for writing distances. Abort.\n");
                		myexit(1);
                				}
	...
	if ( VERBOSE )
		...
		if ( HAVE_TORSION_FILE == YES )
		{
			...
			if ( in == NULL )
				{
        	                        printf("_REDB_Can not open file to read torsion tetraplets._DEF_\n");
					myexit( 1 );
				}
			while ( fgets( string, 299, in ) != NULL )
			{
				...
				if ( tot_tetras > MAX_NOF_BACK-2 )
					{
						printf("_REDB_Too many atom tetraplets for torsions. Increase MAX_NOF_BACK._DEF_\n");
						myexit(1);
								}
				}
			...
			if ( VERBOSE )
			{
				printf("_GREENB_Read %d tetraplets for torsion calculation._DEF_\n", tot_tetras );
			}
		}
		else
		{
		...
		tors = fopen("carma.torsions", "w");
	        if ( tors == NULL )
        	        {
                		printf("_REDB_Can not open file for writing torsions. Abort.\n");
                		myexit(1);
                				}
	...
	if ( VERBOSE )
		...
		if ( HAVE_BEND_FILE == YES )
		{
			...
			if ( in == NULL )
				{
        	                        printf("_REDB_Can not open file to read triplets for bending angles._DEF_\n");
					myexit( 1 );
				}
			while ( fgets( string, 299, in ) != NULL )
			{
				...
				if ( tot_triplets > MAX_NOF_BACK-2 )
					{
						printf("_REDB_Too many atom triplets for angles. Increase MAX_NOF_BACK._DEF_\n");
						myexit(1);
								}
				}
			...
			if ( VERBOSE )
			{
				printf("_GREENB_Read %d triplets for bending angles' calculation._DEF_\n", tot_triplets );
			}
		}
		else
		{
	...
	if ( VERBOSE )
		...
		if ( QFRACTION == NO  &&  ((float)(min2(headerframes, LAST)) - firstframe +1.0)/(float)(STEP) > 100.0 )
			{
			printf("_REDB_If the DCD header is correct, this will create %d postscript files._DEF_\n", 
	...
	if ( VERBOSE )
		...
		if ( QFRACTION == YES && HAVE_REF == NO )
			{
				if ( HAVE_PDB_QFRACTION == YES && REFERENCE > 1 )
					{
						printf("_REDB_Both -ref and -nat defined for Qfraction ? Abort._DEF_\n");
						myexit(1);
					}
				...
				atoms_from_pdb = get_pdb_coords( QFRACT_PDB );
				
				if ( atoms_from_pdb != nofCAs )
					{
				...
				if ( REFERENCE > 1 )
				{
				if ( lseek( dcd, (off_t)(bytes_per_set * (off_t)(REFERENCE-2)), SEEK_CUR ) == (off_t)(-1) )
					{
						printf("_REDB_Requested reference frame not present ? Abort._DEF_\n");
						myexit(1);
					}
				...
				for ( k=i ; k <= nofCAs ; k++ )
					if ( strcmp( str_segname[i-1], str_segname[k-1] ) != 0 )
						{							
						printf("_REDB_\nCalculation of fraction of native contacts only supported for a single chain._DEF_\n\n");
						myexit(1);
						}
				...
				if ( tot_for_qfraction < 2 || tot_for_qsfraction < 2 )
					{							
					printf("_REDB_\nOnly %d distances entered the calculation of native contacts ? Abort._DEF_\n\n", tot_for_qfraction);
					myexit(1);
					}
				
				if ( VERBOSE )
				{
				printf("_YELLOW_%d pairs entered the calculation of the fraction of native contacts.\n_DEF_", tot_for_qfraction );
				printf("_YELLOW_%d pairs entered the calculation of the fraction of the similarity measure.\n_DEF_", tot_for_sfraction );
				printf("Now processing frame %8d", frame);
				}
				...
				if ( (unsigned int)(read( dcd, (void *)(&dcd_frame[1]), (size_t)(bytes_per_set) )) != bytes_per_set )
					{
						printf("_REDB_Can't seek back to beginning of data ? Abort._DEF_\n");
						myexit(1);
					}
				build_CAs( (int)(nofatoms) + 2);
				max = Dist();
...
/*****************************************************************************************************************************/
	...
	{
		if ( VERBOSE )
			printf("_YELLOW_Taking the logarithm base %f of the distribution._DEF_\n", LAMBDA );
	...
	cns = fopen( filename, "w");
        if ( cns == NULL )
                {
                printf("_REDB_Can not open map file for writing. Abort.\n");
                myexit(1);
                }
	...
	if ( VERBOSE && ATOMS_OUTSIDE > 0 )
		printf("_REDB_\nAtoms outside unit cell detected %d times._DEF_\n", ATOMS_OUTSIDE);
...
/*****************************************************************************************************************************/
	...
	if ( pddf_file == NULL )
	  {
	    printf("_REDB_Can not open file for writing pair distance distribution. Abort.\n");
	    myexit(1);
	  }
...
/*****************************************************************************************************************************/
	...
	fmap_index = fopen( "fmap.index", "r");
        if ( fmap_index == NULL )
                {
                printf("_REDB_Can not open the \"fmap.index\" index file for reading. Abort.\n");
                myexit(1);
                }
	...
	i = 0;
        ...
        while ( fgets( string, 299, fmap_index ) != NULL )
		{
			if ( sscanf( string, "%d", &fmapindeces[i]) != 1 )
				{	
					printf("_REDB_\n\nThe 'fmap.index' file is not valid. Abort._DEF_\n");
					myexit(1);
							}
			fmapindeces[i] = fmapindeces[i] - 1;
			if ( fmapindeces[i] < 0 )
				{
					printf("_REDB_\n\nNOTE : carma expects 1-based indeces in the fmap.index file._DEF_\n");
					myexit(1);
							}

			if ( fmapindeces[i] < 0 )
				{	
					printf("_REDB_\n\nNegative index in the 'fmap.index' file ? Abort._DEF_\n");
					myexit(1);
							}
	...
	if ( lseek( dcd, (off_t)(afterheader), SEEK_SET ) == (off_t)(-1) )
		{
			printf("_REDB_Can not seek back to the beginning of file ? Abort._DEF_\n");
			myexit(1);
					}
...
/* and then start modifying the CA coordinates only and writing out the whole block */
	...
	if ( outdcd == -1 )
		{
			printf("_REDB_Can not open DCD file for output. Abort._DEF_\n");
			myexit( 1 );
					}
	...
	*((unsigned int *)(&outdcdhead[8])) = (unsigned int)(1 + (LAST - firstframe) / STEP);


	if ( (off_t)(write( outdcd, outdcdhead, (size_t)(afterheader) )) != afterheader )
		{
			printf("_REDB_Failed to write header block to DCD file ? Abort._DEF_\n");
			myexit(1);
					}
	if ( lseek( dcd, (off_t)(start_ref), SEEK_SET ) == (off_t)(-1) )
		{
			printf("_REDB_Can not seek back to the reference frame ? Abort._DEF_\n");
			myexit(1);
					}
	...
	if ( (unsigned int)(read( dcd, (void *)(&dcd_frame[1]), (size_t)(bytes_per_set) )) != bytes_per_set )
		{
			if ( VERBOSE )
				printf("_REDB_\nCan not read reference frame ? Abort._DEF_\n");
			myexit(1);
					}
	...
	if ( rms_evolution == NULL )
		{	
			printf("_REDB_Can not open output file. Abort._DEF_\n");
			myexit(1);
					}
	...
	while( frame <= LAST )
		...
		for ( i=0 ; i < fmap_nofatoms ; i++ )
			{
				...
				{
					printf("_REDB_Too large index in \"fmap.index\". Abort._DEF_\n" );
					myexit(1);
							}
		...
		if ( WRITE_CAS == YES )
			...
			if ( (unsigned int)(write( outdcd, (void *)(&wdcd_frame[1]), (size_t)(wbytes_per_set) )) != wbytes_per_set )
				{
					printf("_REDB_Failed to write frame %d to DCD file._DEF_\n", frame );
					myexit(1);
							}
								}
		else
...
/*****************************************************************************************************************************/
	...
	rotate_t 	rot_matrix;
	xlate_t		trans_matrix;
	static FILE	*rms_evolution;
	static float	*matrixline;
	float		sssize;
	float		max_of_mins;
	int		i;
	...
	if ( VERBOSE == YES && MAXMIN == NO )
		printf("Calculate RMSD matrix for trajectory.\n");

	if ( VERBOSE == YES && MAXMIN == YES )
		printf("Calculate max of mins of RMSD matrix of the trajectory.\n");

	if ( MAXMIN == YES )
		{
		int i;
		
		matrixline = vector( 1, (LAST-FIRST+1)/STEP + 2 );
		for ( i=1 ; i <= (LAST-FIRST+1)/STEP + 2 ; i++ )
			matrixline[i] = 100000000000.0;
		}

	if ( (sssize = ((float)(LAST) - FIRST +1.0)/(float)(STEP)) > 10000.0 && MAXMIN == NO )
		{
		printf("_REDB_If the DCD header is correct, this will create a %4.2f MBytes matrix file._DEF_\n", 
				sssize*sssize*6 / (1024*1024) );
				
#ifndef WINDOWS
		sleep(2);
#endif	
		if ( VERBOSE && sssize > 20000.0 )
			printf("_YELLOW_OK. This will be a depressingly long calculation ..._DEF_\n");
		}
	...
	/************************************/									
	for ( REFERENCE = REFFIRST ; REFERENCE <= REFLAST ; REFERENCE += STEP )
	{
	...
	while( frame <= LAST )
		...
		if ( MAXMIN == NO )
			fprintf(rms_evolution, " %5.3f", fabs(current_rms) );

		if ( MAXMIN == YES )
		{
			if ( current_rms > 0.000010 && current_rms < matrixline[ entered+1] )
				matrixline[ entered+1] = current_rms;
		}
	...
	if ( MAXMIN == YES )
	{
	rms_evolution = fopen("carma.RMSD.mins", "w");
	if ( rms_evolution == NULL )
		{	
			printf("_REDB_Can not open output file. Abort._DEF_\n");
			myexit(1);
	...
	max_of_mins = matrixline[ 1 ];
	for ( i=1 ; i <= entered ; i++ )
		{
			fprintf( rms_evolution, " %6.3f\n", matrixline[i] );
			if ( matrixline[i] > max_of_mins )
				max_of_mins = matrixline[i];
		}
	fclose(rms_evolution);
	printf("_YELLOW_Max of mins of RMSD matrix is %f._DEF_\n", max_of_mins );
	}
...
/*****************************************************************************************************************************/
/**	torsion RMSD matrix (TCROSS Keyword) 										    **/
/*****************************************************************************************************************************/
	...
	if ( VERBOSE == YES && MAXMIN == NO )
		printf("Calculate torsion RMSD matrix for trajectory.\n");

	if ( VERBOSE == YES && MAXMIN == YES )
		printf("Calculate max of mins of torsion RMSD matrix of the trajectory.\n");
	...
	if ( MAXMIN == NO )
	{
	rms_evolution = fopen("carma.t-RMSD.matrix", "w");
	if ( rms_evolution == NULL )
		{	
			printf("_REDB_Can not open output file. Abort._DEF_\n");
			myexit(1);
	...
	REF_xyz = vector( 1, nof_torsions );
	FIT_xyz = vector( 1, nof_torsions );
	...
	while( frame <= LAST )
		...
		for ( i=0 ; i < nof_torsions ; i++ )
	        ...
	        d = 0.0;
	        for ( i = 0 ; i < nof_torsions ; i += 2 )
	          {
	            a21_a11 = REF_xyz[i+1] - FIT_xyz[i+1];
	            a22_a12 = REF_xyz[i+2] - FIT_xyz[i+2];
	        ...
	        current_rms = sqrt( d / (nof_torsions/2) ) ;
	...
	if ( MAXMIN == YES )
	{
	rms_evolution = fopen("carma.t-RMSD.mins", "w");
	if ( rms_evolution == NULL )
		{	
			printf("_REDB_Can not open output file. Abort._DEF_\n");
			myexit(1);
...
/*****************************************************************************************************************************/
	...
	while( frame <= LAST )
		...
		if ( entered > totentered )
			{
				printf("_REDB_\nThe DCD file is still growing ? Use the \"-last\" flag. Aborting._DEF_\n");
				myexit( 1 );
			}
		build_CAs( (int)(nofatoms) + 2);
...
*/
	...
	if ( bins < 4 )
		{
			printf("_REDB_\nToo few (%d) bins for DeltaG plots. Abort._DEF_\n", bins);
			myexit(1);
		}
	if ( bins < 30 && VERBOSE )
		{
			printf("_REDB_\nWarning: very few (%d) bins for DeltaG plots._DEF_\n", bins);
		}
	...
	if ( DG_PLOTS >= 3 ) 
		{
		bins2 =freedman( tframes );
		if ( VERBOSE )
			printf("Freedman-Diaconis estimate : initial %d, ", bins2 );		
		if ( bins2 > 4 * (int)((int)( pow( (double)( tframes), 1.0/3.0 ) +0.50) / 2 +0.50) )
			bins2 = 4 * (int)((int)( pow( (double)( tframes), 1.0/3.0 ) +0.50) / 2 +0.50);			
		if ( VERBOSE )
			printf("set to %d.\n", bins2 );
		...
		for ( frame = 1 ; frame <= tframes ; frame++ )
			{
				...
				pos1 = (int)( (bins2-1) * (val1 + limit) / (2.0*limit) + 0.50 )+1;
				pos2 = (int)( (bins2-1) * (val2 + limit) / (2.0*limit) + 0.50 )+1;
				pos3 = (int)( (bins2-1) * (val3 + limit) / (2.0*limit) + 0.50 )+1;
	...
	for ( frame = 1 ; frame <= tframes ; frame++ )
	{
		...
		pos1 = (int)( (bins-1) * (val1 + limit) / (2.0*limit) + 0.50 )+1;
		pos2 = (int)( (bins-1) * (val2 + limit) / (2.0*limit) + 0.50 )+1;
		...
		for ( i=0 ; i < nofCAs ; i++ )
			{
				...
				if ( 	xint < 8 || yint < 8 || zint < 8 ||
					...
					{
						printf("_REDB_\nSurface bug: index outside range (%d %d %d) ? Abort._DEF_\n",
								xint, yint, zint );
						myexit(1);
								}
...
/*****************************************************************************************************************************/
	...
	if ( VERBOSE && STRIDE == YES )
		printf("_YELLOW_Will attempt to produce secondary structures using STRIDE._DEF_\n");

	if ( ((float)(min2(headerframes, LAST)) - firstframe +1.0)/(float)(STEP) > 100.0 && STRIDE == NO )
		{
		printf("_REDB_If the DCD header is correct, this will create %d PDB files._DEF_\n", 
				(int)(((float)(min2(headerframes, LAST)) - firstframe +1.0)/(float)(STEP)) );
#ifndef WINDOWS
		sleep(2);
#endif
	...
	while( frame <= LAST )
		...
		for ( i=0 ; i < nofCAs ; i++ )
			{	
							/* Get stride to work with C-capped */
			if ( STRIDE == YES && str_resname[i][0] == 'N' && str_resname[i][1] == 'M' && str_resname[i][2] == 'E' && str_atname[i][0] == 'C' && str_atname[i][1] == 'H' && str_atname[i][2] == '3' )
			fprintf(outPDB, "ATOM% 7d  CA  %s %s%4d    % 8.3f% 8.3f% 8.3f  1.00  0.00\n", 
				i+1, str_resname[i], str_segname[i], str_resid[i],
		...
		if ( HAVE_CHECKED_STRIDE == NO )
		{
			...
			if ( check != 0 )
				{
					printf("_REDB_\n\nAttempt to use stride failed. Abort._DEF_\n");
					myexit(1);
				}
...
/*****************************************************************************************************************************/
	...
	NEW_STEP = nearint(((float)(min2(headerframes, LAST)) - firstframe +1.0)/500.0);
	if ( NEW_STEP < 1 )
		NEW_STEP = 1;
	...
	if ( outPDB == NULL )
		{	
			printf("_REDB_Can not open carma.superposition.pdb file. Abort._DEF_\n");
			myexit(1);
					}
	...
	while( frame <= LAST )
		...
		if ( (frame-firstframe) % NEW_STEP == 0 && frame > firstframe )
		{		
		entered++;		
		build_CAs( (int)(nofatoms) + 2);

		fprintf(outPDB, "MODEL%9d\n", current_model );
		current_model++;
...
/*****************************************************************************************************************************/
	...
	if ( EIGENFIRST_W > 0 && (EIGENFIRST_W > eigendim || EIGENLAST_W > eigendim ))
		{
			printf("_REDB_A requested eigenvector does not exist._DEF_\n");
			myexit( 1 );
					}
...
/*****************************************************************************************************************************/
	...
	if ( ART_EIGEN > eigendim )
		{
			printf("_REDB_Too many eigenvectors requested._DEF_\n");
			myexit( 1 );
					}
	if ( USE_DOT == YES )
		{
			printf("_REDB_Can not construct artificial trajectory with \"-dot\" products._DEF_\n");
			exit ( 1 );
					}
...
/* chosen value from the given range */
	...
	if ( proj == NULL )
		{	
			printf("_REDB_Can not open the 'carma.fluctuations.dat' file. Abort._DEF_\n");
			myexit(1);
					}
	                  ...
	                  for (i=1 ; i <= ART_EIGEN ; i++ )
	                          {
	                                  if ( fscanf( proj, "%f", &sum ) == EOF )
	                                          {
	                                          printf("_REDB_Unexpected end-of-file for 'carma.fluctuations.dat'. Abort._DEF_\n");
	                                          exit ( 1 );
	                                          }
...
*/
	...
	if ( LABELS == YES && i % 5 == 0 )
		{
			char    atnumber_string[100];

			
			sprintf( atnumber_string, "%d %s %s", str_resid[ realCAs[i] ], str_resname[ realCAs[i] ], str_segname[ realCAs[i] ] );

			if ( GRA_VIEW == 3 )
				cmov2(x+XOFFSET, y+YOFFSET);
			else if ( GRA_VIEW == 1 )
				cmov2(y+XOFFSET, z+YOFFSET);
			else
				cmov2(z+XOFFSET, x+YOFFSET);
			
		        charstr(atnumber_string);
...
char 	name[];
{
	...
	if ( in == NULL )
		{
			printf("_REDB_Can not open .psf file for read. Abort.\n");
			myexit( 1 );
					}
	...
	if ( out == NULL )
		{
			printf("_REDB_Can not open file for writing selected atoms. Abort.\n");
			myexit( 1 );
					}
	}
	...
	if ( fgets( string, 299, in ) != NULL )
		{
			...
			if ( 	strncasecmp( &strings[1][0], "EXT", 3) == 0 ||
				...
				{
				if ( VERBOSE )
				printf("_GREENB_PSF with the EXT extension detected._DEF_\n");
		...
		{
			printf("_REDB_Empty PSF file ? Abort._DEF_\n");
			myexit( 1 );
		}
	...
	if ( nof == 0 )
		{
			printf("_REDB_PSF file without an NATOM card ? Abort._DEF_\n");
			myexit( 1 );
					}
	...
	for ( i=0 ; i < nof ; i++ )
		...
		if ( fgets( string, 299, in ) == NULL )
			{
				printf("_REDB_Corrupt .psf file ? Abort._DEF_\n");
				myexit( 1 );
						}
		
		if ( strlen( string ) < 2 )
			{
				printf("_REDB_Empty line in .psf file while reading atom lines ? Abort._DEF_\n");
				myexit( 1 );			
			}
		...
		if ( strncasecmp( &string[INDEX_1], &ATOMID[natm*5], 5) == 0 || ALL_ATID == YES || (HEAVY==YES && strncasecmp( &string[INDEX_11], "H", 1) != 0) )
			...
			if ( WRITE_DATA == YES )
				...
				if ( EXTENDED == NO )
				{
				sprintf(string, "%8d", nof2+1 );
				string[8] = ' ';
				fprintf(out, "%s", string );
				}
				else
				{
				sprintf(string, "%10d", nof2+1 );
				string[10] = ' ';
				fprintf(out, "%s", string );
				}
				}
			...
			if ( k == ATOM_TYPES )
				{
					...
					/* printf("_RED_Unknown atom type in the following PSF line :\n  %s_DEF_", string); */

					masses[nof2] = atof( &string[INDEX_6] );

					/* if ( !EXTENDED )
					printf("_RED_Got a value of %f from the PSF file._DEF_\n\n", masses[nof2] ); */
			...
			if ( nof2 == MAX_NOF_CAS - 1 )
				{
					printf("_RED_Maximum number of atoms exceeded. Change MAX_NOF_CAS and re-compile._DEF_\n");
					myexit(1);
							}
								}
									}
		...
		if ( strncasecmp( &string[INDEX_22], &SEGID[nseg*5], 5) == 0 && (strncasecmp( &string[INDEX_1], &ATOMID[natm*5], 5) == 0 || ALL_ATID == YES || (HEAVY==YES && strncasecmp( &string[INDEX_11], "H", 1) != 0)))
			...
			if ( nof2 == MAX_NOF_BACK - 2 )
				{
					printf("_RED_Maximum number of backbone atoms exceeded. Change MAX_NOF_BACK and re-compile._DEF_\n");
					myexit(1);
							}
...
/*
	if ( VERBOSE && UNKNOWN )
		printf("_RED_Unknown atom types detected in the PSF. Continuing.\n_DEF_");
*/

	if ( ZERO_MASSES == YES )
		printf("_YELLOW_Some atoms' masses were zero. TIP4P maybe ???\n_DEF_");
...
char	name[];
{
	...
	out = fopen( newname, "w" );
        if ( out == NULL )
                {
                printf("_REDB_Can not open data file for writing. Abort.\n");
                myexit(1);
                }
	if ( VERBOSE )
	...
	if ( out == NULL )
		{
		printf("_REDB_Can not open postscript file for writing. Abort.\n");
		myexit(1);
		}
	if ( VERBOSE )
...
char	name[];
{
	...
	if ( in == NULL )
		{
			printf("_REDB_Can not open .pdb file for read. Abort.\n");
			myexit( 1 );
					}
		...
		if ( HAVE_PDB_QFRACTION == YES )
		{
		if ( strncasecmp( &string[13], &str_atname[nof][0], 1 ) != 0 )
			{
			printf("\n_REDB_Mismatch between PSF and native structure's PDB ? Abort._DEF_\n\n");
			myexit(1);
			}
		}
...
{
	printf("\n\n%s\n\n", &error_text[0] );
	printf("_REDB_Allocation errors mean that carma requested additional_DEF_\n");
	printf("_REDB_memory and the operating system refused to offer it._DEF_\n");
	printf("_REDB_If your computing machine does have enough memory for_DEF_\n");
	printf("_REDB_the envisioned calculation, getting the 64bit GNU/Linux_DEF_\n");
	printf("_REDB_carma executable may be the solution to this problem._DEF_\n\n");
...
void	filter()
{
	...
	if ( nofCAs * nofCAs != ndata )
		{
			printf("_REDB_The %d values read-in do not make a square matrix._DEF_\n", ndata);
			myexit( 1 );
					}
	...
	{
	if ( REVERSE == NO )
		printf("_GREEN_Will plot the gradient from %f (white) to %f (black)._DEF_\n", gmin, gmax );
	else
		printf("_GREEN_Will plot the gradient from %f (black) to %f (white)._DEF_\n", gmin, gmax );

	REVERSE = !REVERSE;

	}
	else
		printf("_GREEN_Will plot the gradient from %f to %f using colour postscript._DEF_\n", gmin, gmax );
...
/**********************************************************************************************************/	
	...
	if ( EIGEN == YES )
	{
	printf("_WHITEB_Eigenvalues and eigenvectors of the input matrix will be calculated.\n");
	printf("If the input matrix is not symmetric you will get _REDB_wrong_WHITEB_ results._DEF_\n");
...
char 	*name1, *name2;
{
	...
	if ( in1 == NULL )
		{
			printf("_REDB_Can not open the file \"%s\" for read. Abort.\n", name1 );
			myexit( 1 );
					}
	if ( in2 == NULL )
		{
	...
	if ( out == NULL )
		{
			printf("_REDB_Can not open the carma.merged.ps file for output. Abort.\n");
			myexit( 1 );
					}
        ...
        while ( fgets( string1, 299, in1 ) != NULL && fgets( string2, 299, in2 ) != NULL )
        	...
        	if ( strcmp( string1, string2 ) != 0 )
        		{
        		printf("_REDB_The postscript files' headers differ. Abort._DEF_\n");
        		myexit(1);
        			}
	...
	c22 = getc(in2);
 	...
 	if ( c11 == EOF || c12 == EOF || c21 == EOF || c22 == EOF )
 		{
 			printf("_REDB_Unexpected end of file encountered. Abort._DEF_\n");
 			myexit( 1 );
 					}
	if ( y >= x )
		{
...
char 	*listname;
{
	...
	if ( list == NULL )
		{
			printf("_REDB_Can not open file containing the frame number order._DEF_\n");
			myexit(1);
					}
	...
	if ( frame < 0 )
		{
			printf("_REDB_Frame numbers must be positive integers greater than zero._DEF_\n");
			myexit(1);
					}
	if ( lseek( dcd, (off_t)(start), SEEK_SET ) == (off_t)(-1) )
		{
			printf("_REDB_Can not seek the beginning of data ? Abort._DEF_\n");
			myexit(1);
					}
	...
	if ( (unsigned int)(read( dcd, (void *)(&dcd_frame[1]), (size_t)(bytes_per_set) )) != bytes_per_set )
		{
			printf("_REDB_Requested frame not found. Abort._DEF_\n");
			myexit(1);
					}
	if ( (unsigned int)(write( outdcd, (void *)(&dcd_frame[1]), (size_t)(bytes_per_set) )) != bytes_per_set )
		{
	...
	if ( VERBOSE )
	printf("_GREENB_Wrote %d frames out of %d in the DCD file._DEF_\n", frames_written, headerframes);
...
float	**data;
{
	...
	if ( USE_LAST == YES )
	{
	printf("_WHITEB_Re-using eigenvalues and eigenvectors from previous run._DEF_\n");
	...
	if ( in == NULL )
		{
		printf("_REDB_Can not read eigenvectors file. Abort._DEF_\n");
		myexit(1);
			}
	for ( i=1 ; i <= eigendim ; i++)
	for ( k=1 ; k <= eigendim ; k++)
		if ( fscanf(in, "%f", &eigenvectors[i][k]) != 1 )
			{
			printf("_REDB_Failure while reading from the eigenvectors file. Abort._DEF_\n");
			myexit(1);
			}
	...
	if ( DPCA == NO )
        ...
        if ( in == NULL )
                {
                printf("_REDB_Can not open eigenvalues file. Abort._DEF_\n");
                myexit(1);
                        }
	...
	for ( i=1 ; i <= eigendim ; i++ )
		if ( fscanf(in, "%f", &eigenvalues[i] ) != 1 )
                        {
                        printf("_REDB_Failure while reading from the eigenvalues file. Abort._DEF_\n");
                        myexit(1);
                        }
	...
	ssyev_( &JOBZ, &UPLO, &N, &data[1][1], &LDA, &eigenvalues[1], &extra[1], &LWORK, &INFO);
  	if ( INFO != 0 ) 
  		{
    			printf("_REDB_Error in LAPACK ssyev() call: %d_DEF_\n",INFO);
			myexit(1);
  					}
	...
	if ( out == NULL )
		{
		printf("_REDB_Can not open eigenvectors file for output. Abort._DEF_\n");
		myexit(1);
		}
	for ( i=1 ; i <= eigendim ; i++)
	{
	...
	if ( DPCA == NO )
        ...
        if ( out == NULL )
                {
                printf("_REDB_Can not open eigenvalues file for output. Abort._DEF_\n");
                myexit(1);
                }
	for ( i=1 ; i <= eigendim ; i++ )
		fprintf(out, " %15.10f\n", eigenvalues[i] );
	...
	printf("_REDB_\n\nEntropy calculation will ignore negative eigenvalues !_DEF_\n");
	
	printf("\nEntropy (Andricioaei) using only %6d eigenvalues is %f (J/molK) \n", last, entropy );
	...
	entropy = 0.0;
	for ( i=1 ; i <= eigendim -6 ; i++ )
		if ( CONST2 * Temp * M_E * M_E * eigenvalues[i] + 1.0 > 0.0 )
			entropy += log1p( CONST2 * Temp * M_E * M_E * eigenvalues[i] );
		else
			break;
	...
	if ( i >= eigendim -6 )
		...
		printf("Entropy (Schlitter)   using only %6d eigenvalues is %f (J/molK) \n", i, entropy );
	}
...
int n;
{
	...
	for (l=1;l<=n;l++) {
		...
		do {
			...
			if (m != l) {
				if (iter++ == 200) nrerror("Too many iterations in tqli");
				g=(d[l+1]-d[l])/(2.0*e[l]);
				r=pythag(g,1.0);
...
static void hinds_matrix(rotate_t u, xlate_t t,
  ...
  for (l = 0; l < 2; l++) {
    ...
    d = sqrt(d);
    if (d == 0) {
    			printf("_REDB_Rotation matrix determination failed. Abort._DEF_\n");
    			myexit(1);
    					}
...
float bestfit(rotate_t u, xlate_t t,
  ...
  float	*aa=0, *bb=0;
	...
	if ( indeces == NULL )
		{	
			printf("_REDB_\nCan not open the 'fit.index' file. Abort._DEF_\n");
			myexit(1);
					}
	...
	i = 0;
        while ( fgets( string, 299, indeces ) != NULL )
		{
			...
			if ( sscanf( string, "%d", &index[i]) != 1 )
				{	
					printf("_REDB_\n\nThe 'fit.index' file is not valid. Abort._DEF_\n");
					myexit(1);
							}
			index[i] = index[i] - 1;
			if ( index[i] < 0 )
				{
					printf("_REDB_\n\nNOTE : carma expects 1-based indeces in the fit.index file._DEF_\n");
					myexit(1);
							}

			if ( index[i] >= npts )
				{	
					printf("_REDB_\n\nToo large index in the 'fit.index' file. Abort._DEF_\n");
					myexit(1);
							}
			i++;
			}
	...
	if ( i > npts )
		{	
			printf("_REDB_Too many indeces in the 'fit.index' file. Abort._DEF_\n");
			myexit(1);
					}
  ...
  sw = solve(e, e0, det, spur, cof);
	...
	if ( sw != 1 )
		{
                printf("_REDB_ Internal problem with bestfit(). Continuing (but beware) ..._DEF_\n");
                }
...
int	build_torsions()
{
	...
	if ( nofCAs % 3 != 0 )
		{
			printf("_REDB_Number of N,C,CA atoms not a multiple of three ? Abort._DEF_\n");
			myexit( 1 );
					}
	if ( nofCAs < 6 )
		{
			printf("_REDB_Too few N,C,CA atoms ? Abort._DEF_\n");
			myexit( 1 );
					}
	...
	for ( i=0 ; i < nofCAs ; i++ )
		{
			if ( 	strncmp( &backbone  [i][5], " N   ", 5) != 0	&&
				...
				strncmp( &backbone  [i][5], " C   ", 5) != 0	)
					{
					printf("_REDB_Unexpected atom for torsions : %s_DEF_\n" ,&backbone[i][5] );
					myexit( 1 );
					}
	...
	for ( i=0 ; i < nofCAs ; i += 3 )
	{
		...
		while( str_resid[i+3] == str_resid[i]+1 && strcmp( str_segname[i+3], str_segname[i] ) == 0 )
		{
			int	N1=0, CA1=0, C1=0;
			int	N2=0, CA2=0, C2=0;
		...
		if ( i < nofCAs -  3 )
			{
				if ( strcmp( str_segname[i+3], str_segname[i] ) == 0 )
					{
/*
						i += 3;
*/
						if ( VERBOSE )
							printf("_YELLOW_Break in chain detected during torsion preparation. Skipping ..._DEF_\n");
					}
			}
	}
	...
	if ( tot_torsions < 2 )
		{
			printf("_REDB_Less than two torsions left after selection ? Abort._DEF_\n");
			myexit( 1 );
					}
	

	if ( VERBOSE && DPCA == YES )
	printf("_YELLOW_Starting dPCA with %d torsion angles_DEF_\n", tot_torsions);

	if ( VERBOSE && TCROSS == YES )
	printf("_YELLOW_t-RMSD matrix calculation : %d torsion angles included_DEF_\n", tot_torsions);
...
int	build_torsions_with_chi()
{
	...
	if ( nofCAs < 6 )
		{
			printf("_REDB_Too few atoms selected for torsions ? Abort._DEF_\n");
			myexit( 1 );
					}
...
*/
	...
	{
								/* While the next triplet belongs to the next residue, do ... */
		while( str_resid[ index_back[i+3] ] == str_resid[ index_back[i] ]+1 && strcmp( str_segname[ index_back[i+3] ], str_segname[index_back[i]] ) == 0 )
		{
			int	N1=0, CA1=0, C1=0;
			int	N2=0, CA2=0, C2=0;
		...
		if ( i < nof_back -  3 )
			{
				if ( strcmp( str_segname[ index_back[i+3] ], str_segname[index_back[i]] ) == 0 )
					{
/*					
						i += 3;
*/
	...
	if ( VERBOSE && DPCA == YES )
		{
		printf("_YELLOW_Starting dPCA with %d torsion angles_DEF_\n", tot_torsions);
		printf("_YELLOW_of which %d are backbone and %d are chi1 angles._DEF_\n", tot_back, tot_side );
		}
	if ( VERBOSE && TCROSS == YES )
		{
		printf("_YELLOW_t-RMSD matrix calculation : %d torsion angles_DEF_\n", tot_torsions);
		printf("_YELLOW_of which %d are backbone and %d are chi1 angles._DEF_\n", tot_back, tot_side );
		}
...
/*************************************************************************/
/* TESTING : use tanh() to reduce the height of the denser peaks so that */
/*           minor peaks can also be picked-up                           */
/*************************************************************************/
	...
	if ( current_max > 999998.0 )
		{
			printf("_REDB_\n\nMore than 1 million frames on a single map point ? Abort._DEF_\n");
			exit( 1 );
		}
	...
	for ( k=1 ; k <= bins ; k++ )	
	for ( m=1 ; m <= bins ; m++ )	
		inter[i][k][m] = (MAP_AVER+20.0*MAP_RMSD)*tanh( inter[i][k][m] / (MAP_AVER+20.0*MAP_RMSD) );
...
/*************************************************************************/
	...
	if ( out == NULL )
		{	
			printf("_REDB_Can not open a 'carma.variance_explained.dat' file for writing. Abort._DEF_\n");
			myexit(1);
					}
	...
	else
	{
		...
		/* v.1.7 : even more disgusting heurestics ... */
		if ( max_pos < max_clust-3 )
		{
			     if ( (variances[ max_pos+3 ][0] - variances[ max_pos ][0]) > 0.150 )
				max_pos = max_pos+3;
			else if ( (variances[ max_pos+2 ][0] - variances[ max_pos ][0]) > 0.100 )
				max_pos = max_pos+2;
			else if ( (variances[ max_pos+1 ][0] - variances[ max_pos ][0]) > 0.050 )
				max_pos = max_pos+1;
		}
		...
		if ( variances[ max_pos ][0] < 0.75 && max_pos < max_clust-1 )		
		{
			...
			if ( (variances[ max_pos2 ][0] > 0.80) || (variances[ max_pos2 ][0] - variances[ max_pos ][0] > 0.10 ) )
				{
					CLU_RMS = variances[ max_pos2 ][1];
					if ( VERBOSE )
					printf("Using the second-best solution to obtain a higher variance explained.\n");
	...
	if ( MAP_VARIANCE / ( GRID_POINTS * MAP_RMSD * MAP_RMSD) < 0.65 )
		printf("_YELLOW_Variance explained is low. Consider using a different cluster analysis method._DEF_\n");
	...
	for ( i = 1 ; i <= frames ; i++ )
	{
		...
		pos1 = (int)( (bins-1) * (val1 + limit) / (2.0*limit) + 0.50 )+1;
		pos2 = (int)( (bins-1) * (val2 + limit) / (2.0*limit) + 0.50 )+1;
		pos3 = (int)( (bins-1) * (val3 + limit) / (2.0*limit) + 0.50 )+1;
...
int	bins;
{
	...
	{
		printf("_REDB_\n\nCluster analysis made a boo boo! Goodbye._DEF_\n");
		printf("_YELLOW_[this probably means that the number of\nframes used for the analysis is too small]._DEF_\n");
		exit(1);
	}
	...
	for ( frame = 1 ; frame <= N ; frame++ )
			{
				...
				val2 = DG[frame][2];
				val3 = DG[frame][3];
				norms[ frame ] = sqrt( val1*val1 + val2*val2 + val3*val3 );
			}
	sort( N, norms );
	min = norms[1];
	max = norms[N];	
	h_N = (int)( (max-min) / (2*(norms[ (int)(3.0*N/4.0 + 0.5)]-norms[ (int)(N/4.0 +0.5) ]) / pow( N, 1.0l/3.0l)) + 0.50 );
	free_vector( norms, 1, N );
	return( h_N );
}
...
float	calc_min_dist()
{
	...
	z = CAs[k][2];
        ...
        yy = y + a4*orthom00+a5*orthom01+a6*orthom02;
        zz = z + a7*orthom00+a8*orthom01+a9*orthom02;
        if ( (dist=sqrt((xxx-xx)*(xxx-xx)+(yyy-yy)*(yyy-yy)+(zzz-zz)*(zzz-zz))) < min )
        	min = dist;
